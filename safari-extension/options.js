...
function formatRbResult(result, title) {
  ...
  const lines = [
    title,
    `Safari 文件夹: ${result.chromeFolders || 0}，Safari 书签总数: ${result.totalChromeBookmarks || 0}`,
    ...
    `待同步到 Safari: ${c.toAddCount || 0}${result.preview ? ' (预览)' : `，已写入 ${c.addedToChrome || 0} 条`}`,
  ];
  const toAdd = result.samples?.toAdd || [];
  if (toAdd.length) {
    lines.push('待推送到 Safari:');
