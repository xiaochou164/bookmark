...
function formatRainbowResult(result, title) {
  ...
  const lines = [
    title,
    `--- Safari 侧 ---`,
    `Safari 文件夹数: ${result.chromeFolders || 0}`,
    `Safari 书签总数: ${result.totalChromeBookmarks || 0}`,
    `新增到 Safari: ${c.toAddCount || 0}${result.preview ? ' (预览，未写入)' : ` → 已写入 ${c.addedToChrome || 0} 条`}`,
    `从 Safari 删除: ${c.toDeleteCount || 0}${result.preview ? ' (预览，未删除)' : ` → 已删除 ${c.deletedFromChrome || 0} 条`}`,
    ``,
    `--- 云端 DB 侧 ---`,
...
rbPreviewBtn.addEventListener('click', async () => {
  ...
  showStatus('预览中（Safari ↔ 云书签 Rainbow）...', 'info');
...
rbSyncBtn.addEventListener('click', async () => {
  setBusy(ALL_BTNS, true);
  showStatus('同步中（Safari ↔ 云书签 Rainbow）...', 'info');
