...
function registerIoRoutes(app, deps) {
  ...
  app.get('/api/io/tasks', async (req, res, next) => {
    try {
      ...
      const tasks = await ioTasks.listTasks({ userId, limit, status, type });
  ...
  app.get('/api/io/tasks/:taskId', async (req, res, next) => {
    try {
      const userId = String(req.auth?.user?.id || '');
      const task = await ioTasks.getTask(String(req.params.taskId), { userId });
  ...
  app.post('/api/io/tasks', async (req, res, next) => {
    try {
      ...
      const task = await ioTasks.enqueueTask({ userId, type, input });
  ...
  app.post('/api/io/tasks/:taskId/retry', async (req, res, next) => {
    try {
      const userId = String(req.auth?.user?.id || '');
      const task = await ioTasks.retryTask(String(req.params.taskId), { userId });
      res.status(202).json({ ok: true, task });
    } catch (err) {
      const msg = String(err?.message || err || '');
